...
6. 字段映射明细：每次出站执行前记录 CRM 字段、接口字段、表参数行号、实际取值、是否使用默认值和是否缺失，便于排查推送值是否正确。
7. 返回成功判断：接口定义可配置 `AUTO`、`HTTP_STATUS`、`TEXT_CONTAINS`、`JSON_FIELD`、`XML_FIELD`、`SAP_RETURN`，保留返回原文，同时按消息或字段值判断执行是否成功。`SAP_RETURN` 同时支持单结构和表结构返回，表结构会扫描多行 `TYPE/MESSAGE`，任意一行 `E/A/X` 即判失败，并汇总多行消息。
8. 异常重推：失败数据通过日志进入重推队列，重推时增加重试次数并重置状态。
