...
3. 接口定义：维护接口编码、接口名称、系统编码、连接编码、协议、方向、业务模块、SAP 函数/BAPI、HTTP 方法、接口路径、内容类型、启用状态和重试次数。
4. 字段映射：维护来源字段、目标字段、字段类型、是否必填、默认值、转换规则、排序和备注。
...
现状：已新增集成平台基础模型和页面入口，支持通用连接配置、SAP RFC 配置、接口定义、字段映射、平台日志和异常重推。SAP RFC 目前完成配置和参数校验，真实连通性需接入 SAP JCo 或企业已有 RFC 网关执行器。
...
3. 建立标准执行链路：业务触发、字段映射、报文生成、接口调用、响应解析、日志落库、失败重试。
4. 字段映射支持转换函数，例如日期格式转换、字典映射、固定值、组合字段、空值兜底。
...
3. 新增接口定义，设置接口编码、协议、方向、业务模块、函数名或接口路径。
4. 配置字段映射，明确 CRM 字段、外部字段、字段类型、必填、默认值和转换规则。
...
4. 字段映射可维护来源字段、目标字段、类型、必填、默认值和转换规则。
